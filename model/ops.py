...
def batch_norm(x, is_training, epsilon=1e-5, decay=0.9):
    return tf.nn.batch_norm(x, decay=decay, updates_collections=None, epsilon=epsilon,
...
def conv2d(x, output_filters, kh=5, kw=5, sh=2, sw=2, stddev=0.02):

    shape = x.get_shape().as_list()
    W = tf.Variable('W', [kh, kw, shape[-1], output_filters],
                        initializer=tf.truncated_normal_initializer(stddev=stddev))
    Wconv = tf.nn.conv2d(x, W, strides=[1, sh, sw, 1], padding='SAME')

    biases = tf.Variable('b', [output_filters], initializer=tf.constant_initializer(0.0))
    Wconv_plus_b = tf.reshape(tf.nn.bias_add(Wconv, biases), Wconv.get_shape())
...
def deconv2d(x, output_shape, kh=5, kw=5, sh=2, sw=2, stddev=0.02):

    # filter : [height, width, output_channels, in_channels]
    input_shape = x.get_shape().as_list()
    W = tf.Variable('W', [kh, kw, output_shape[-1], input_shape[-1]],
                        initializer=tf.random_normal_initializer(stddev=stddev))

    deconv = tf.nn.conv2d_transpose(x, W, output_shape=output_shape,
                                    strides=[1, sh, sw, 1])

    biases = tf.Variable('b', [output_shape[-1]], initializer=tf.constant_initializer(0.0))
    deconv_plus_b = tf.reshape(tf.nn.bias_add(deconv, biases), deconv.get_shape())
...
def fc(x, output_size, stddev=0.02):

    shape = x.get_shape().as_list()
    W = tf.Variable("W", [shape[1], output_size], tf.float32,
                        tf.random_normal_initializer(stddev=stddev))
    b = tf.Variable("b", [output_size],
                        initializer=tf.constant_initializer(0.0))
    return tf.matmul(x, W) + b
...
def init_embedding(size, dimension, stddev=0.01):
    return tf.Variable("E", [size, 1, 1, dimension], tf.float32,
                                   tf.random_normal_initializer(stddev=stddev))
'''
...
def conditional_instance_norm(x, ids, labels_num, mixed=False):

    shape = x.get_shape().as_list()
    batch_size, output_filters = shape[0], shape[-1]
    scale = tf.Variable("scale", [labels_num, output_filters], tf.float32, tf.constant_initializer(1.0))
    shift = tf.Variable("shift", [labels_num, output_filters], tf.float32, tf.constant_initializer(0.0))
